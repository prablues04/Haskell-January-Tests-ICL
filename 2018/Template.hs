...
update assignment@(var,value) vars@(v:vs)
    | var == fst v = assignment:vs
    | otherwise    = v: update assignment vs
...
eval (Apply op e1 e2) vars = apply op (eval e1 vars) (eval e2 vars)
eval (Var x) vars          = lookUp x vars
eval (Const x) _           = x

execStatement :: Statement -> State -> State
execStatement (Assign id expr) vars = update (id, eval expr vars) vars
execStatement (If expr b1 b2) vars
    | eval expr vars == 1 = execBlock b1 vars
    | otherwise           = execBlock b2 vars
execStatement (DoWhile block expr) vars
    | eval expr vars' == 1 = execStatement (DoWhile block expr) vars'
    | otherwise            = vars'
      where vars' = execBlock block vars


execBlock :: Block -> State -> State
execBlock [] vars = vars
execBlock block@(statement:statements) vars = execBlock statements vars'
    where
        vars' = execStatement statement vars
...
foldConst (Phi (Const c) (Const d))
    | c == d    = (Const c)
    | otherwise = (Phi (Const c) (Const d))
foldConst (Apply op (Const c) (Const d)) = Const (apply op c d)
foldConst (Apply Add e1 e2)
    | e1 == (Const 0) = e2
    | e2 == (Const 0) = e1
foldConst expr = expr
...
sub :: Id -> Int -> Exp -> Exp
-- Pre: the expression is in SSA form
sub varName varVal expr = foldConst (sub' expr)
    where
        sub' (Var x)
            | varName == x = Const varVal
            | otherwise    = Var x
        sub' (Apply op e1 e2) = Apply op (sub' e1) (sub' e2)
        sub' (Phi e1 e2)      = Phi (sub' e1) (sub' e2)
        sub' (Const x)        = Const x
...
propagateConstants block
    ...
    | otherwise               = propagateConstants (newBlock block)
    where
        getVars [] vars = vars
        getVars ((Assign id expr@(Const x)):statements) vars = getVars statements vars'
            where vars' = [(id, x)] ++ vars
        getVars (_:statements) vars = getVars statements vars

        constVars = filter (\(x, y) -> x/="$return") (getVars block [])

        substitute :: Statement -> (Id,Int) -> Statement
        substitute (If expr b1 b2) (variable, value)
            = If (foldConst (sub variable value expr)) (propagateConstants b1) (propagateConstants b2)
        substitute (DoWhile block' expr) (variable, value)
            = DoWhile (propagateConstants block') (foldConst (sub variable value expr))
        substitute statement _ = statement
        newBlock lastBlock@(s:ss)
            | constVars == [] = []
            | otherwise       = removeAssignments (substitute s (head constVars):propagateConstants ss)
        removeAssignments [] = []
        removeAssignments b@(s@(Assign v _):s')
            | v `notElem` (map fst constVars) = s:removeAssignments s'
            | otherwise                       = removeAssignments s'  
        removeAssignments b@(s:s') = s:removeAssignments s'
