...
tryToLookUp x d list
  = fromMaybe d (lookup x list)
...
unifyPairs ps s
    ...
    | otherwise               = Just finalS
      where
        finalS = foldr (unifyPair) s (concatMap addToList ps)
        occursInSub = [occurs x y | (x,y) <- finalS]
        unifyPair :: (Type, Type) -> Sub -> Sub
        unifyPair (t, t') s'
            | t == t' = s'
        unifyPair (TVar v, TVar v') s'
            = [(v,TVar v'), (v', TVar v)] ++ s'
        unifyPair (t@(TVar v), t') s'
            = (v,t'):s'
        unifyPair (t,t'@(TVar v')) s'
            = (v',t):s'
        --in all other cases the test fails, so adding the exception case will also result in failure
        unifyPair _ s' 
            = ("a", TVar "a"):s'
