...
assignArray arr@(A bindings) i@(I index) v@(I val)
    = A ((index,val):filter (\(x,_) -> x /= index) bindings)

updateVar :: (Id, Value) -> State -> State
updateVar pair@(var,val) bindings
    | var `elem` globals = (var, (Global, val)):filteredBindings 
    | otherwise          = (var, (Local, val)):filteredBindings 
    where 
        filteredBindings = filter (\(x,_) -> x /= var) bindings
        globals = map (\(x,_) -> x) (getGlobals bindings)
...
applyOp Equal (I v1) (I v2)
    | v1 == v2  = I 1
    | otherwise = I 0
applyOp Index (A arr) (I var) = I (fromMaybe 0 (lookup var arr))

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs vars vals = zip vars (map (\x -> (Local, x)) vals)

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs exprs fundefs state
    = map (\x -> eval x fundefs state) exprs
...
-- Pre: For FunApp, the given function exists in the list of function definitions given
eval (Const x) _ _   
    = x
eval (Var x) _ state 
    = getValue x state
eval (OpApp op v1 v2) fundefs state  
    = applyOp op (eval v1 fundefs state) (eval v2 fundefs state)
eval (Cond pred v1 v2) fundefs state
    | eval pred fundefs state == I 1 = eval v1 fundefs state
    | otherwise                      = eval v2 fundefs state
eval (FunApp func es) fundefs state = eval e fundefs bindings 
    where 
        evaluatedEs = evalArgs es fundefs state
        (as,e) = lookUp func fundefs
...
executeStatement (Assign var exp) fds _ state 
    = updateVar (var, eval exp fds state) state
{-executeStatement (AssignA id e1 e2) fds _ state
    = updateVar (id, (scope,assignArray arr (eval e1 fds state) (eval e2 fds state)))
      where (scope,arr) = lookUp id state-}
executeStatement (If expr b1 b2) fds pds state
    | eval expr fds state == I 1 = executeBlock b1 fds pds state
    | otherwise                  = executeBlock b2 fds pds state
...
executeBlock block fds pds state
    = foldr (\x -> executeStatement x fds pds) state block
